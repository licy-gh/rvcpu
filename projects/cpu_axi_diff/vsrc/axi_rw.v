...
module axi_rw # (
    ...
    wire apb_addr_rena;
    wire apb_addr_wena;
    assign apb_addr_rena = (r_addr_i[31:12] == `UART_ADDR || r_addr_i[31:12] == `SPI_CTRL_ADDR || r_addr_i[31:28] == `FLASH_ADDR);
    assign apb_addr_wena = (w_addr_i[31:12] == `UART_ADDR || w_addr_i[31:12] == `SPI_CTRL_ADDR || r_addr_i[31:28] == `FLASH_ADDR);
    ...
    assign icache_ready  = axi_r_valid_i;
   ...
   /* reg    aw_valid;
    ...
    always @(posedge clock) begin
        ...
        else if(w_ena == `TRUE_V && aw_hs == `FALSE_V) begin
            aw_valid <= `TRUE_V;
        end
        else if(aw_hs == `TRUE_V) begin
            aw_valid <= `FALSE_V;
        end
    end */


    assign axi_aw_valid_o   = (w_ena == `TRUE_V && w_addr_i != `MTIMECMP_ADDR && w_addr_i != `MTIME_ADDR) ? w_state_addr : `FALSE_V;
    assign axi_aw_addr_o    = {w_addr_i[AXI_ADDR_WIDTH-1:ALIGNED_WIDTH], {ALIGNED_WIDTH{1'b0}}};
    ...
    assign axi_ar_valid_o   = (r_valid_i) ? r_state_addr : `FALSE_V;
    assign axi_ar_addr_o    = (r_len_i) ?   r_addr_i    :
                              (apb_addr_rena == `TRUE_V)    ?   {r_addr_i[31:2],2'b0}   :   axi_addr;
    ...
    assign axi_ar_len_o     = (r_len_i != 2'b00)    ?   8'b0000_0001    :   
                              (apb_addr_rena == `TRUE_V)    ?   8'b0    :   axi_len;
    assign axi_ar_size_o    = (r_len_i != 2'b00)    ?   r_size_i        :
                              (apb_addr_rena == `TRUE_V)    ?   3'b010  :   axi_size;
    ...
    wire [63: 0] r_data_i_apb_select =  (size_w)    ?   {32'b0,axi_r_data_i[31: 0]} :
                                        (size_h == `TRUE_V & r_addr_i[1] == 1'b0)   ?   {48'b0,axi_r_data_i[15: 0]} :
                                        (size_h == `TRUE_V & r_addr_i[1] == 1'b1)   ?   {48'b0,axi_r_data_i[31:16]} :
                                        (size_b == `TRUE_V & r_addr_i[1:0] == 2'b0)     ?   {56'b0,axi_r_data_i[ 7: 0]} :
                                        (size_b == `TRUE_V & r_addr_i[1:0] == 2'b01)    ?   {56'b0,axi_r_data_i[15: 8]} :
                                        (size_b == `TRUE_V & r_addr_i[1:0] == 2'b10)    ?   {56'b0,axi_r_data_i[23:16]} :
                                        (size_b == `TRUE_V & r_addr_i[1:0] == 2'b11)    ?   {56'b0,axi_r_data_i[31:24]} :   `ZERO_WORD;
    ...
    assign data_read_o =(r_len_i != 2'b00)  ?   axi_r_data_i    :
                        (apb_addr_rena == `TRUE_V) ?   data_read_o_apb : data_read_o_ddr;
